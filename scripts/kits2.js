...
onChat((cmdObject) => {
    try {
        if (cmdObject.content === test[0]) {
			...
			system.executeCommand(`execute @a[name="${playerName}"] ~ ~ ~ tellraw @a {"rawtext":[{"text":"§bKits are: .kit archer, .kit miner, .kit lumberjack, .kit warrior, .kit mage, §a(.kit staff)§b and .kit demolitionist"}]}`, () => {});
        }
